...
tools = ["dynamatic-opt", "export-dot", "hls-fuzzer", "hls-fuzzer-check-bitwidth",
         ...
         ToolSubst("%export-verilog",
                   command=f"rm -rf %t; mkdir %t; {config.dynamatic_tools_dir}/export-rtl %s %t {config.dynamatic_src_root}/data/rtl-config-verilog.json --dynamatic-path {config.dynamatic_src_root} --hdl verilog"),
         ToolSubst("%export-smv",
                   command=f"rm -rf %t; mkdir %t; {config.dynamatic_tools_dir}/export-rtl %s %t {config.dynamatic_src_root}/data/rtl-config-smv.json --dynamatic-path {config.dynamatic_src_root} --hdl smv"),

  ...
  if (auto commitOp = dyn_cast<handshake::SpecCommitOp>(currOp)) {
    ...
    for (auto [valueForSpecTag, branchOp, branchDirection] : branchTrace) {
      ...
      if (!branchDiscardNonSpec.has_value()) {
        ...
        branchDiscardNonSpec = handshake::SpeculatingBranchOp::create(
            builder, branchOp.getLoc(),
      ...
      if (!branchReplicated.has_value()) {
        ...
        branchReplicated = handshake::ConditionalBranchOp::create(
            builder, branchDiscardNonSpec->getLoc(),
...
LogicalResult HandshakeSpeculationPass::placeCommits() {
  ...
  fakeControlForCommits =
      mlir::UnrealizedConversionCastOp::create(
          builder, speculatorOp->getLoc(), commitCtrl.getType(), ValueRange{})
          .getResult(0);
  ...
  for (OpOperand *operand : placements.getPlacements<SpecCommitOp>()) {
    ...
    // later.
    SpecCommitOp newOp = SpecCommitOp::create(
        builder, dstOp->getLoc(), /*resultType=*/srcOpResult.getType(),
...
LogicalResult HandshakeSpeculationPass::placeSaveCommits() {
  ...
  for (OpOperand *operand : placements.getPlacements<SpecSaveCommitOp>()) {
    ...
    SpecSaveCommitOp newOp = SpecSaveCommitOp::create(
        builder, dstOp->getLoc(), /*resultType=*/srcOpResult.getType(),
...
LogicalResult HandshakeSpeculationPass::placeSpeculator() {
  ...
  // later.
  speculatorOp = SpeculatorOp::create(
      builder, dstOp->getLoc(), /*dataOutType=*/srcOpResult.getType(),
    ...
    if (dataResultType.hasExtraSignal(EXTRA_BIT_SPEC)) {
      ...
      for (auto dataOperand : mergeLikeOp.getDataOperands()) {
        ...
        if (!dataOperandType.hasExtraSignal(EXTRA_BIT_SPEC)) {
          ...
          auto nonSpecOp =
              NonSpecOp::create(builder, mergeLikeOp.getLoc(),
                                dataOperand.getType(), dataOperand);

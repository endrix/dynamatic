from generators.support.arith_utils import *
from generators.support.utils import *


def generate_divui(name, params):
    latency = params[ATTR_LATENCY]
    impl = params.get(ATTR_IMPL, "pipelined")
    data_type = SmvScalarType(params[ATTR_BITWIDTH])
    abstract_data = params[ATTR_ABSTRACT_DATA]

    if abstract_data:
        return generate_abstract_binary_op(name, latency, data_type, impl)
    else:
        return _generate_divui(name, latency, data_type, impl)


def _generate_divui(name, latency, data_type, impl):
    return f"""
{generate_binary_op_header(name)}
  DEFINE result := lhs / rhs;
  
  {generate_binary_op_handshake_manager(f"{name}__handshake_manager", {ATTR_LATENCY: latency, ATTR_IMPL: impl})}
"""

...
static Value lowerVariadicAndInverterOp(AndInverterOp op, OperandRange operands,
  ...
  case 1:
    if (inverts[0])
      return synth::AndInverterOp::create(rewriter, op.getLoc(), operands[0],
                                          true);
    else
      return operands[0];
  case 2:
  ...
  default:
    ...
    return synth::AndInverterOp::create(rewriter, op.getLoc(), lhs, rhs);
  }
  return Value();
}

LogicalResult
dynamatic::synth::AndInverterVariadicOpConversion::matchAndRewrite(
...
// Parsing function for assembly format of AndInverterOp
ParseResult dynamatic::parseVariadicInvertibleOperands(
    OpAsmParser &parser,
...
// Printing function for assembly format of AndInverterOp
void dynamatic::printVariadicInvertibleOperands(
    OpAsmPrinter &printer, Operation *op, OperandRange operands,
    Type resultType, mlir::DenseBoolArrayAttr inverted,

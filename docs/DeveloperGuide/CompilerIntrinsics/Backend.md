...
- `sram_macros`: the macros on hand, a Python list of `(name, words, width)` triples, for instance `[('sky130_sram_1rw1r_64x256_8',256,64),('sky130_sram_1rw1r_44x64_8',64,44)]` (no spaces: the generator's parameters are split on them). The smallest macro that holds the memory is taken, the fewest words first and then the narrowest, and padded with zeros where it is deeper or wider; a memory no macro holds stays flops, with a note on stderr. FakeRAM generates a macro of any size, so the list is optional for it.
...
The OpenRAM view serves the store on the read-write port and the load on the read port, so a load and a store in one cycle are both served, the load reading the old word as the flop model does; chip select and write enable are active low, and every byte lane of the write mask is written. OpenRAM launches its read data on the falling edge of the cycle that sampled the address, so `dout1` is valid before the next rising edge, where the flop model's registered `loadData` would be sampled: the same cycle, with half of it for the path from the macro to the reader.

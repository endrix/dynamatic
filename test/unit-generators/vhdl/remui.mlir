...
// The pipelined unsigned remainder unit wraps the urem core from
// vitis_hls_cores.vhd under its own wrapper name, at the channel's width,
// and its valid travels through the same latency buffer the divisions use
// (bitwidth + 3 cycles). IMPL names it: the sequential unit is
// divsi-remui-remsi-impl.mlir's.
module {
  ...
  hw.module.extern @handshake_remui_0(in %lhs : !handshake.channel<i32>, in %rhs : !handshake.channel<i32>, in %clk : i1, in %rst : i1, out result : !handshake.channel<i32>) attributes {hw.name = "handshake.remui", hw.parameters = {DATA_TYPE = !handshake.channel<i32>, IMPL = "pipelined", LATENCY = 35 : ui32}}
}

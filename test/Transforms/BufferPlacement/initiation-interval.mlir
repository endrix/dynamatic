...
// faster than a token every 9 cycles, and one slot carries the product.
// Without the interval the placer models the same unit as pipelined, gives
// the loop a token a cycle, and buffers the multiplier's output for the
// nine tokens it believes are in flight.
//
// REQUIRES: cbc
